...
initiate_aks_deployment() {
  ...
  yq eval-all 'select(fileIndex == 0) * select(fileIndex == 1) | del(.. | select(. == null))' "${DIR}/value_files/${HELM_CHART_VALUE_FILE_NAME}" "${DIR}/value_files/${HELM_CHART_AKS_DIFF_VALUE_FILE_NAME}" > "/tmp/${HELM_CHART_AKS_MERGED_VALUE_FILE_NAME}"
...
initiate_rbac_aks_deployment() {
  ...
  yq eval-all 'select(fileIndex == 0) * select(fileIndex == 1) | del(.. | select(. == null))' "${DIR}/value_files/${HELM_CHART_RBAC_VALUE_FILE_NAME}" "${DIR}/value_files/${HELM_CHART_RBAC_AKS_DIFF_VALUE_FILE_NAME}" > "/tmp/${HELM_CHART_RBAC_AKS_MERGED_VALUE_FILE_NAME}"
